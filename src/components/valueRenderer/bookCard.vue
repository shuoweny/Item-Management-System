...
onBeforeMount(async () => {
  ...
  data.categoryName.splice(0);
  props.category.forEach((id) => {
    const result = data.categoryList.find((element) => element.id == id);
    data.categoryName.push(result.name);
  })
  data.categoryString = data.categoryName.join();
});
nextTick(() => {
